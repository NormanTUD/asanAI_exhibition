...
var default_model_struct = [
	{conv2d: {filters: 4, activation: "relu", kernelInitializer: _kernel_initializer, biasInitializer: _bias_initializer, kernelSize: [3, 3], inputShape: [width_and_height, width_and_height, 3] }},
...
async function load_and_train_scheine_muenzen_schluessel() {
	...
	return await _load_example("scheine_muenzen_schluessel", "second_example_images", 50, model_struct, default_optimizer_config, ["Scheine", "Münzen", "Schlüssel"]);
}

async function load_and_train_fruits_example() {
	return await _load_example("fruits", "first_example_images", 50, default_model_struct, default_optimizer_config, ["apfel", "banane", "orange"]);
}
	...
	for (var first_key_idx = 0; first_key_idx < _keys.length; first_key_idx++) {
		...
		for (var second_key_idx = 0; second_key_idx < _keys.length; second_key_idx++) {
			...
			if(_first_key != _second_key) {
				var new_line = `${nr_string} ${trm("images_of_category")} <b>${_first_key_uppercase}</b> ${trm("was_detected_as_category")} <b>${_second_key_uppercase}</b>.<br>\n`;
